...
export const update_encryption = async (res,cid,UIDc)=>{
    ...
    if(finalres.length){
       ...
       const pubKeyCre= finalres[0].CrePubKey;
       const id=finalres[0]._id;
      ...
      const encid=encryptedcid.toString('base64');
      //update in mongo db

     
       
    
        pkg.findByIdAndUpdate(id, {EncCid:encid})
          // .then(res.send("encid Updated Successfully..."))
          .catch((err)=> console.log(err))
    }
    else{console.log("no record found to update!!!");}
