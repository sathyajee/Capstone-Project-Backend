...
//--------------------------nomini keys pair-----------------------------------------------------
const { publicKey :nomini_publicKey, privateKey: nomini_privateKey } = crypto.generateKeyPairSync("rsa", {
    // The standard secure default length for RSA keys is 2048 bits
    modulusLength: 8192,
...
const encryptedcid = crypto.publicEncrypt(
  ...
  const enCreatorPrivKey = crypto.publicEncrypt(
    ...
    // We convert the data string to a buffer using `Buffer.from`
    Buffer.from(creator_privateKey.export({ type: 'pkcs1', format: 'pem' }).toString('base64'))
  );
  ...
  const encrypted_data = {
    ...
    creator_publicKey: creator_publicKey.export({ type: 'pkcs1', format: 'pem' }).toString('base64'),   
    nomini_publicKey: nomini_publicKey.export({ type: 'pkcs1', format: 'pem' }).toString('base64')
}
